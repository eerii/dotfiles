...
with lib;
with builtins; {
  config = mkIf config.wayland.enable (mkIf osConfig.sway.enable {
    wayland.windowManager.sway = {
      ...
      config = let
        ...
        jumpToWorkspace = listToAttrs (map (v: {
          name = "${mod}+${toString v}";
          value = "workspace number ${toString v}";
        }) workspaces);
        moveToWorkspace = listToAttrs (map (v: {
      ...
      in rec {
        ...
        output = {
          ...
          "DP-1" = let
            resX =
              strings.toInt (elemAt (split "x" osConfig.sway.resolution) 0);
            scaledResX =
              elemAt (split "." (toString (resX / osConfig.sway.scale))) 0;
          in {
        ...
        keybindings = mkMerge [
          ...
          moveToWorkspace
          {
            ...
            # Run app
            "${mod}+space" =
              "exec pkill rofi || rofi -show drun -show-icons | xargs swaymsg exec --";
            # Open directory
            "Ctrl+space" = ''
              exec pkill rofi || ${terminal} -D "$(zoxide query --list | ${menu})"'';
            # Show clipboard history
            "${mod}+v" =
              "exec pkill rofi || cliphist list | ${menu} | cliphist decode | wl-copy";
            ...
            "XF86AudioRaiseVolume" =
              "exec wpctl set-mute @DEFAULT_AUDIO_SINK@ 0 && wpctl set-volume @DEFAULT_AUDIO_SINK@ 5%+ -l 1.0";
